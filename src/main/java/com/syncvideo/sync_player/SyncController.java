...
public class SyncController {
    ...
    public void joinRoom(@DestinationVariable String roomId, @Payload VideoMessage message,
        ...
        if (room.isLocked) {
            messagingTemplate.convertAndSendToUser(headerAccessor.getSessionId(), "/topic/errors",
                    new VideoMessage("ERROR_LOCKED", "System", "Room is Locked", 0.0, 0.0));
            return;
        }
    ...
    public void toggleLock(@DestinationVariable String roomId, @Payload VideoMessage message) {
        ...
        if (room != null && message.getSender().equals(room.owner)) {
            ...
            messagingTemplate.convertAndSend("/topic/room/" + roomId, new VideoMessage("LOCK_UPDATE", "System",
                    room.isLocked ? "LOCKED" : "UNLOCKED", 0.0, room.isLocked ? 1.0 : 0.0));
        }
    }
    ...
    // Helper: Validates files and sends READY/WAIT/ERROR messages
    private void validateAndNotify(String roomId) {
        ...
        for (Double d : usersInRoom.values())
            if (d > 0)
                activeVideoUsers++;
        ...
        if (activeVideoUsers > 0 && allMatch) {
            if (activeVideoUsers < room.users.size()) {
                messagingTemplate.convertAndSend("/topic/room/" + roomId,
                        new VideoMessage("WAIT", "Server", "Friend selecting video...", 0.0, 0.0));
            } else {
                messagingTemplate.convertAndSend("/topic/room/" + roomId,
    ...
    // Helper: Pure Boolean Check Logic
    private boolean validateFilesInternal(String roomId) {
        ...
        return true; // All valid durations match
    }
